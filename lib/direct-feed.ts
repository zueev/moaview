...
import type {Listing} from './feed-parser';
export type SourceState={name:string;url:string;status:'ok'|'error';count:number;scope:string};
export type DirectListing=Listing&{image?:string;kind:'배송형'|'방문형';endAt?:string;category:Category;categoryOrigin:'source'|'summary'};
const text=(v:unknown)=>String(v??'').replace(/<script[\s\S]*?<\/script>/g,'').replace(/<[^>]+>/g,' ').replace(/&amp;/g,'&').replace(/&gt;/g,'>').replace(/&lt;/g,'<').replace(/&nbsp;/g,' ').replace(/&quot;/g,'"').replace(/\s+/g,' ').trim();
const get=(s:string,re:RegExp)=>text(s.match(re)?.[1]);
const seoul=/^서울(?:\s|\/|$|특별시)/;
const localSeoul=/^(삼성역|강남역|잠실|합정|연남|성수|홍대|선릉|역삼|신촌|여의도|종로|한남|이태원|신당|건대|망원|서초|사당|마곡|목동|노원|성신여대|서울대입구)(?:\s|$)/;
function entry(platform:string,url:string,title:string,region:string,benefit:string,deadline:string,image:string,applicants:number|null,recruits:number|null,delivery=false):DirectListing|null{
 if(!deadline||/종료|마감완료|선정완료/.test(deadline))return null;
 if(!delivery&&!seoul.test(region)&&!localSeoul.test(region))return null;
 return {category:classify(title,benefit),categoryOrigin:'summary',id:platform+':'+url,platform,url,title,name:title.replace(/^\[[^\]]+\]\s*/,''),region:delivery?'전국 배송':seoul.test(region)?region:'서울 · '+region,benefit,conditions:'상세 미션·이용 조건은 원문에서 확인해 주세요.',deadline,channel:'블로그',applicants,recruits,image:/^https:\/\//.test(image)?image:undefined,kind:delivery?'배송형':'방문형'};
}
export function reviewnote(html:string):DirectListing[]{const match=html.match(/<script id="__NEXT_DATA__" type="application\/json">([\s\S]*?)<\/script>/);if(!match)throw Error('목록 형식 확인 필요');const data=JSON.parse(match[1]).props.pageProps.data;const images=[...html.matchAll(/src="([^"]+)"/g)].map(m=>text(m[1]));return (data.objects as any[]).flatMap(x=>{if(x.channel!=='BLOG'||!['SELECT','APPLY'].includes(x.status)||Date.parse(x.applyEndAt)<Date.now())return [];const delivery=x.sort==='DELIVERY';if(delivery&&/보험|컨설팅|상담사 출장|물품 배송x|포인트만/.test(x.offer))return [];const region=x.city+' '+(x.sido?.name||'');const days=Math.max(0,Math.ceil((Date.parse(x.applyEndAt)-Date.now())/86400000));const image=images.find(u=>u.includes(encodeURIComponent(x.imageKey)))||'';const r=entry('리뷰노트','https://www.reviewnote.co.kr/campaigns/'+x.id,x.title,region,text(x.offer),days===0?'오늘 마감':`D-${days}`,image,x.applicantCount,x.infNum,delivery);if(r&&['뷰티','패션','식품','생활','리빙'].includes(x.category?.title)){r.category=({'뷰티':'beauty','패션':'fashion','식품':'food','생활':'living','리빙':'living'} as Record<string,Category>)[x.category.title];r.categoryOrigin='source';}if(r&&/지역.*참여|지역.*가능/.test(x.title)){r.region='배송형 · 지역 제한';r.conditions=x.title;}return r?[{...r,endAt:x.applyEndAt}]:[]});}
...
const KOREA=/^(서울|경기|인천|부산|대구|대전|광주|울산|세종|강원|충북|충남|전북|전남|경북|경남|제주)/;
export function reviewplace(html:string):DirectListing[]{return html.split("<div class='item'>").slice(1).flatMap(block=>{const s=block.split('</a>')[0];if(!/blog_icon/.test(s))return [];const path=get(s,/href='([^']+)'/);if(!path)return [];const title=get(s,/class='tit'>([\s\S]*?)<\/p>/);const region=title.match(/^\[([^\]]+)\]/)?.[1]||'';const counts=text(s).match(/신청\s*([\d,]+)\s*\/\s*([\d,]+)명/);const r=entry('리뷰플레이스',new URL(path,'https://www.reviewplace.co.kr/').href,title,region,get(s,/class='txt'>([\s\S]*?)<\/p>/),get(s,/class='date'>([\s\S]*?)<\/p>/).replace(/\s+/g,''),get(s,/<img src="([^"]+)"/),counts?Number(counts[1].replaceAll(',','')):null,counts?Number(counts[2].replaceAll(',','')):null,!KOREA.test(region));return r?[r]:[]});}

// 스토리앤미디어와 리뷰어스는 같은 CMS 스킨을 쓴다.
function skinList(platform:string,base:string){return (html:string):DirectListing[]=>html.split('class="box">').slice(1).flatMap(block=>{const s=block.split('</li>')[0];if(!/class="sns blog"/.test(s))return [];const path=get(s,/href="(review_campaign\.php\?cp_id=[^"]+)"/);if(!path)return [];const title=get(s,/class="it_name">([\s\S]*?)<\/span>/);const region=title.match(/^\[([^\]]+)\]/)?.[1]||'';const image=get(s,/<img src="([^"]+)"[^>]*class="it_img"/);const counts=text(s).match(/신청\s*([\d,]+)\s*\/\s*모집\s*([\d,]+)/);const r=entry(platform,new URL(path,base).href,title,region,get(s,/class="it_description">([\s\S]*?)<\/span>/),get(s,/class="dday">([\s\S]*?)<\/span>/).replace('D-Day ','D-'),image?new URL(image,base).href:'',counts?Number(counts[1].replaceAll(',','')):null,counts?Number(counts[2].replaceAll(',','')):null,/배송형/.test(get(s,/class="option2">([\s\S]*?)<\/div>/)));return r?[r]:[]});}
export function storyn(html:string):DirectListing[]{return skinList('스토리앤미디어','https://www.storyn.kr/')(html);}
